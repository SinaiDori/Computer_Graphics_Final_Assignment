...
function handleKeyDown(e) {
  ...
  if (e.key === " " && !isMovingBall && !chargingShot) {
    e.preventDefault();
    chargingShot = true;
    shotPower = 0;
  }
...
function handleKeyUp(e) {
  ...
  // Release shot
  if (e.key === " " && chargingShot && !isMovingBall) {
    e.preventDefault();
    chargingShot = false;
...
function handleBallMovement() {
  ...
  if (chargingShot && !isMovingBall) {
    shotPower += 0.2; // Steady increase
    if (shotPower > maxShotPower) {
      shotPower = maxShotPower; // Cap at max
    }
    updatePowerMeter();
  }
}
